...
test('getPreviousMonth returns correct previous month and year', () => {
	assert.deepEqual(getPreviousMonth('January', '2026'), {
		year: '2025',
		month: 'December',
	});
	assert.deepEqual(getPreviousMonth('February', '2026'), {
		year: '2026',
		month: 'January',
	});
	assert.deepEqual(getPreviousMonth('December', '2026'), {
		year: '2026',
		month: 'November',
	});
	assert.deepEqual(getPreviousMonth('January', '2020'), {
		year: '2019',
		month: 'December',
	});
...
test('getNextMonth returns correct next month and year', () => {
	assert.deepEqual(getNextMonth('January', '2026'), {
		year: '2026',
		month: 'February',
	});
	assert.deepEqual(getNextMonth('February', '2026'), {
		year: '2026',
		month: 'March',
	});
	assert.deepEqual(getNextMonth('December', '2026'), {
		year: '2027',
		month: 'January',
	});
	assert.deepEqual(getNextMonth('January', '2019'), {
		year: '2019',
		month: 'February',
	});
...
test('getEventsForMonth returns correct events for a given month and year', () => {
	...
	const eventsOctober2020 = getEventsForMonth(daysData, 'October', '2020');
	assert.deepEqual(eventsOctober2020, [
		{ event: 'Ada Lovelace Day', date: 13 },
		{ event: 'World Lemur Day', date: 30 },
	]);

	const eventsSeptember2026 = getEventsForMonth(
		daysData,
		'September',
		'2026',
	);
